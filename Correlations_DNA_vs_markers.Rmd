...
df_medians <- paired_boxes(sce, 'Original')
ggplot(df_medians[signif != ""], aes(x = group, y = Expression, color = group))+
    geom_violin()+
    geom_point()+
    geom_line(aes(group = patient_id), color = '#C4C4C4')+
    scale_color_manual(values = c("MP" = "#009ADE", "RP" = "#FF1F5B"))+
    facet_wrap(~marker_title, scales = 'free', nrow = 3)+
    theme_minimal()+
...
```{r}
sce <- readRDS(file.path(path_to_data, mapping['CD42b']))
df_medians <- paired_boxes(sce, 'CD42b')
ggplot(df_medians[signif != ""], aes(x = group, y = Expression, color = group))+
    geom_violin()+
    geom_point()+
    geom_line(aes(group = patient_id), color = '#C4C4C4')+
...
sce <- readRDS(file.path(path_to_data, mapping['DNA2']))
df_medians <- paired_boxes(sce, 'DNA2')
ggplot(df_medians[signif != ""], aes(x = group, y = Expression, color = group))+
    geom_violin()+
    geom_point()+
    geom_line(aes(group = patient_id), color = '#C4C4C4')+
